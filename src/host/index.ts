	...
	ctx.effect(() => {
				...
				try {
					...
					if (action === "terminal-open") {
						...
						if (process.platform === "win32") {
							...
							const terminal = pty.spawn(process.env.ComSpec || "cmd.exe", ["/d"], {
								name: "xterm-256color", cols, rows, cwd: root, env: process.env
							});
							const record = {
								owner: sessionId,
								process: {
									stdin: { write: (data) => terminal.write(data) },
									kill: () => { if (!record.exited) terminal.kill(); },
									resize: (cols, rows) => terminal.resize(cols, rows)
								},
								chunks: [], exited: false, pendingInput: "", inputTimer: void 0
							};
							terminal.onData((chunk) => record.chunks.push(chunk));
					...
					if (action.startsWith("terminal-")) {
						...
						if (action === "terminal-input") {
							...
							if (record.exited) throw new Error("terminal is unavailable");
							...
							if (record.inputTimer === void 0) {
								record.inputTimer = setTimeout(() => {
									...
									if (!record.exited) record.process.stdin.write(record.pendingInput);
									record.pendingInput = "";
								}, 16);
							}
							return json(res, 200, {
						...
						if (action === "terminal-resize") {
							...
							if (!Number.isFinite(body.cols) || !Number.isFinite(body.rows)) throw new Error("invalid terminal dimensions");
							if (record.exited) throw new Error("terminal is unavailable");
							const applied = typeof record.process.resize === "function";
							if (applied) record.process.resize(Math.min(500, Math.max(2, Math.floor(body.cols))), Math.min(200, Math.max(1, Math.floor(body.rows))));
							// Unix script has no runtime resize channel; ConPTY does.
							return json(res, 200, {
								ok: true,
								accepted: true,
								applied
						...
						if (action === "terminal-close") {
							if (record.inputTimer !== void 0) clearTimeout(record.inputTimer);
							record.inputTimer = void 0;
							record.pendingInput = "";
		...
		return () => {
			...
			for (const terminal of terminals.values()) {
				if (terminal.inputTimer !== void 0) clearTimeout(terminal.inputTimer);
