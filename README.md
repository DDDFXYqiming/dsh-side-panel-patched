...
- 实现入口：`lib/client.js` 中 `dfbRenderMarkdown` / `markdownCss` / `dfbRenderMermaidBlocks`；`src/` 为从 sourcemap 恢复的源码参考
...
- 本包以预构建产物分发（插件实际加载 `lib/`）；`src/` 为从 sourcemap 恢复的源码参考，不参与发布（仓库无构建配置，重建需自备工具链）——`prepare` 脚本仅做产物存在性自检，`npm pack` / GitHub 安装均可通过（publish.md 兼容路径）
